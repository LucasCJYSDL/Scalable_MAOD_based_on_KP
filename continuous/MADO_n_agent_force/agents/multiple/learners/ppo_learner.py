...
class PPOLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if self.args.is_discrete:
            ...
            old_mac_mu, old_mac_sigma = [], []
            for t in range(batch.max_seq_length - 1):
                agent_mus, agent_sigmas = self.old_mac.forward(batch, t=t)
                old_mac_mu.append(agent_mus)
                old_mac_sigma.append(agent_sigmas)
            old_mac_mu = th.stack(old_mac_mu, dim=1)
            old_mac_sigma = th.stack(old_mac_sigma, dim=1)
            m = Normal(old_mac_mu, old_mac_sigma)
            old_log_pi_taken = m.log_prob(actions).sum(-1, keepdim=True)
            old_log_pi_taken[mask == 0] = 0.0 # danger
            old_log_pi_taken = old_log_pi_taken.squeeze(3)
