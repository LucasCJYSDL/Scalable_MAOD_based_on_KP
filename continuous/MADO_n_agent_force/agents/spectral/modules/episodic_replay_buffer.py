...
class EpisodicReplayBuffer:
    ...
    def sample_transitions(self, batch_size):
        episode_indices = self._sample_episodes(batch_size)
        step_ranges = self._gather_episode_lengths(episode_indices)
        step_indices = uniform_sampling(step_ranges) # make sure the last steps of the episodes can be sampled
        s1, a, s2, r, done = [], [], [], [], []
        for epi_idx, step_idx in zip(episode_indices, step_indices): # danger
            temp_transition = self._episodes[epi_idx][step_idx]
            s1.append(temp_transition['s'])
            a.append(temp_transition['a'])
            s2.append(temp_transition['next_s'])
            r.append(temp_transition['r'])
            done.append(temp_transition['done'])

        return s1, a, s2, r, done
