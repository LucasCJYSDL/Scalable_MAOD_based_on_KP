...
class MazeEnv(gym.Env):
    def __init__(
        ...
        **kwargs,
        ...
        self._position_only = self._task.POSITION_ONLY
        ...
        self._init_position = [(x - torso_x, y - torso_y) for x, y in self._find_all_robots()][self.agent_id]
        ...
        xml_path = os.path.join(MODEL_DIR, model_cls.FILE+'_{}.xml'.format(self.agent_id))
        ...
        file_path = os.path.join(MODEL_DIR, model_cls.FILE+'_{}_maze_{}.xml'.format(self.agent_id, datetime.datetime.now().strftime("%m%d_%H%M%S")))
    ...
    def set_episode_limit(self, episode_limit: int) -> None:
    ...
    def _get_obs_space(self) -> gym.spaces.Box:
        ...
        if not self._position_only:
            ...
            high[: 3] = wrapped_obs_space.high[: 3]
            low[: 3] = wrapped_obs_space.low[: 3]
        ...
        if not self._position_only:
            ...
            return gym.spaces.Box(low[:3], high[:3])
    ...
    def set_goal_area(self, goal: np.ndarray, threshold: float) -> None:
        self._task.set_goal_area(goal, threshold)

        tree = ET.parse(self.file_path)
        worldbody = tree.find(".//worldbody")
        worldbody.find(".//site").set("pos", f"{goal[0]} {goal[1]} {0.0}") # attention: don't need to multiply the scale
        tree.write(self.file_path)
        self.wrapped_env = self.model_cls(file_path=self.file_path, **self._kwargs)

    def reset(self, random_init: bool=False, is_sample: bool=False) -> np.ndarray:
        self.t = 0
        # back to MujocoEnv to see this
        self.wrapped_env.reset()
        xy = self.wrapped_env.get_xy()
        if not random_init:
            xy += np.array(self._init_position)
        else:
            if is_sample:
                xy += random.choice(self._task.init_positions())
            else:
                range = self._maze_size_scaling * 0.5 - 0.5
                xy += random.choice(self.empty_blocks)
                xy += np.random.uniform(size=xy.size, low=-range, high=range)
        self.wrapped_env.set_xy(xy) # danger
        # Samples a new goal
    ...
    def get_env_info(self) -> Dict:
        env_info = {"obs_shape": int(np.prod(self.observation_space.shape)),
                    "obs_pos_shape": 2,
                    "action_shape": int(np.prod(self.action_space.shape)),
                    "episode_limit": self.episode_limit if self.episode_limit else 1000}
        return env_info
    ...
    def render(self, mode="human", **kwargs) -> Optional[np.ndarray]: # danger
    ...
    @property
    def action_space(self): # # danger: the output of the NN should be within this range
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, dict]:
        ...
        if self._task.termination([cur_obs]):
            return cur_obs, 1.0, True, {'position': self.wrapped_env.get_xy()}
        ...
        outer_reward = self._task.reward([next_obs]) # no inner reward
    ...
    def is_goal_area(self, loc: np.ndarray) -> bool:
        return self._task.is_goal_area(loc)
