...
def test_offline_agent(args, mdp):
    ...
    for idx in range(agent_num // 2):
        pair_multi_matrix, pair_multi_option, pair_multi_partition = GetOption(mdp, k=n_options, intToS=intToS_list[(idx * 2):(idx * 2 + 2)] ,num_limit=element_limit,
                                                         matrix_list=origMatrix_list[(idx * 2):(idx * 2 + 2)],
                                                         normalized=normalized, with_degree=with_degree, method='multiple', use_median=use_median)
    ...
    single_matrix_list, single_option_list, single_partition_list = GetOption(mdp, k=int(multi_option_num**0.5), with_degree=with_degree, matrix_list=origMatrix_list, method='single', use_median=use_median)
   ...
   #
    ...
    # ['blue', 'red', 'orange']
    run_agents_on_mdp(multi_option_agents, args.task, option_type,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--to_point', type=bool, default=True)
    ...
    parser.add_argument('--ninstances', type=int, default=10)
