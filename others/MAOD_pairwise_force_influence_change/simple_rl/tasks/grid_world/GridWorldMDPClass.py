...
class GridWorldMDP(MDP):
    ...
    def __init__(self, width, height, init_locs, goal_locs, lava_locs, walls,
        ...
        self.collision_ratio = None

    def set_collision_ratio(self, ratio):
    ...
    def _is_goal_state_action(self, states, action_list):  # c
        ...
        next_states, is_terminal = self._transition_func(states, action_list, is_tran=False)
        if is_terminal:
            return True
        else:
            return False

    def _transition_func(self, states, action_list, is_tran=True):  # c
        '''
        Returns
            (State)
        '''
        ...
        if self.is_goal_state(next_states):
            ...
            if np.random.random() > 1.0 - self.collision_ratio:
                # print("here")
                next_states, collision_flag = self.check_collision(next_states, states)
            is_terminal = False

        if is_tran and collision_flag:
            self.collision_num += 1

        return next_states, is_terminal

    def check_collision(self, next_states, states):
        agent_num = len(next_states)
        group_flag = False
        for agent_id in range(agent_num):
            flag = False
            for i in range(agent_id + 1, agent_num):
                if next_states[agent_id].x == states[i].x and next_states[agent_id].y == states[i].y:
                    next_states[agent_id] = GridWorldState(states[agent_id].x, states[agent_id].y)
                    flag = True
                    break
            if not flag:
                for j in range(agent_id):
                    if next_states[agent_id].x == next_states[j].x and next_states[agent_id].y == next_states[j].y:
                        next_states[agent_id] = GridWorldState(states[agent_id].x, states[agent_id].y)
                        flag = True
                        break
            if flag:
    ...
    def visualize_option(self, option_list, intra_policy_list):
        ...
        mdpv.visualize_option(self, _draw_option, option_list, intra_policy_list, action_char_dict)
        input("Press anything to quit")
...
def make_grid_world_from_file(file_name, randomize=False, num_goals=1, name=None, goal_num=None, slip_prob=0.0):  # c
    ...
    return GridWorldMDP(width=num_cols, height=num_rows, init_locs=agent_locs, goal_locs=goal_locs, lava_locs=lava_locs, walls=walls, name=name,
                        slip_prob=slip_prob)
...
if __name__ == '__main__':
    ...
    for _ in range(10):
        action_list = ['up'] * grid_world.get_agent_num()
