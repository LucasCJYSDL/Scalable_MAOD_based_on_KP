...
class MDP(object):
    ...
    def get_obs_state(self):
        temp_state = self.get_curr_states()
        state, obs = [], []
        assert len(temp_state) == len(self.init_states)
        for i in range(len(temp_state)):
            state.append(temp_state[i].x)
            state.append(temp_state[i].y)
            obs.append([temp_state[i].x, temp_state[i].y])
        return np.array(obs), np.array(state)
