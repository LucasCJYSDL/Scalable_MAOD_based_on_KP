...
class Qmix_learner:
    ...
    def log_info(self, delta_IMP, scores, log_dict, itr):
        if delta_IMP:
            self.writer.add_scalar('Step', delta_IMP, itr)
        scores = th.tensor(scores)
        self.writer.add_scalar('Contract Score', scores.mean(), itr)
