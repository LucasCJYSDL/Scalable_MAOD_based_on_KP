...
class ActionAbstraction(object):
    ...
    def __init__(self, options, prim_actions, agent_num, term_prob=0.0, prims_on_failure=False, use_prims=True, gamma=0.99, name=None): # prim: primitive
        ...
        if use_prims:
            if options is not None:
                if self.option_type == 'single':
                    ...
                    for agent_id in range(self.agent_num):
                        self.options.append(self._convert_to_options(prim_actions) + options)
            else:
                for agent_id in range(self.agent_num):
            ...
            if self.option_type == 'single':
                for agent_id in range(self.agent_num):
                    self.options.append(options[agent_id])
            else:
                for agent_id in range(self.agent_num):
    ...
    def get_active_options(self, state):
        ...
        active_options = []
        for agent_id in range(self.agent_num):
            active_options.append([o for o in self.options[agent_id] if o.is_init_true(state, agent_id=agent_id)])
    ...
    def is_next_step_continuing_option(self, ground_state):
        ...
        continue_list = []
        for agent_id in range(self.agent_num):
            continue_list.append(self.is_cur_executing and not self.cur_option[agent_id].is_term_true(ground_state, agent_id=agent_id))
    ...
    def get_next_ground_action(self, ground_state, continue_list):
        ...
        for agent_id in range(self.agent_num):
            if continue_list[agent_id]:
                action_list.append(self.cur_option[agent_id].act(ground_state, agent_id=agent_id))
            else:
    ...
    def abs_to_ground(self, ground_state, option_list):
        ...
        action_list = []
        for agent_id in range(self.agent_num):
            action_list.append(option_list[agent_id].act(ground_state, agent_id=agent_id))
