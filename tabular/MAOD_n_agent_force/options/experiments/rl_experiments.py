...
def build_option_agent(mdp, option_list, partition_list, intToS_list, agent_num, agent_class, option_type, policy='vi', name='-abstr'):
    ...
    goal_based_aa = ActionAbstraction(prim_actions=mdp.get_actions(), options=goal_based_options, use_prims=True,
                                      name=name, gamma=mdp.get_gamma(), agent_num=agent_num) # (high-level policy)
    option_agent = AbstractionWrapper(SubAgentClass=agent_class, agent_params={"avai_action_list": None, "agent_num": agent_num},
...
def test_offline_agent(args, mdp):
    ...
    multi_matrix_list, multi_option_list, multi_partition_list = GetOption(mdp, k=n_options, intToS=intToS_list, num_limit=element_limit,
                                                                           matrix_list=origMatrix_list, normalized=normalized,
                                                                           with_degree=with_degree, method='multiple', use_median=use_median)
    multi_option_num = len(multi_option_list)
    # to make the size of their joint action space more close
    if agent_num < 4:
        k = 2
    else:
        k = 1
    ssingle_matrix_list, single_option_list, single_partition_list = GetOption(mdp, k=k, with_degree=with_degree,
   ...
   #
    cent_agents = CentQLearningAgent(avai_action_list=[mdp.get_actions() for _ in range(agent_num)], agent_num=agent_num)
    # [multi_option_agents, single_option_agents, cent_agents]
    # [args.agent+'_multi_option', args.agent+'_single_option', 'CentQ']
    # ['blue', 'red', 'orange']
    # run_agents_on_mdp([multi_option_agents, cent_agents],
    #                   [args.agent+'_multi_option', 'CentQ'],
    #                   ['blue', 'orange'], args.task, option_type,
    ...
    run_agents_on_mdp([multi_option_agents, single_option_agents, cent_agents],
                      [args.agent+'_multi_option', args.agent+'_single_option', 'CentQ'],
                      ['blue', 'red', 'orange'], args.task, option_type,
                      mdp, instances=n_instances,
                      episodes=n_episodes, steps=n_steps, reset_at_terminal=False, visualize=visualize)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ninstances', type=int, default=3)
