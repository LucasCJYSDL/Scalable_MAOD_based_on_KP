...
def run_agents_on_mdp(agents_list, agent_name_list, agent_color_list, task_name, mdp, instances=5, episodes=100, steps=200, reset_at_terminal=False, visualize=False):
    ...
    for idx in range(len(agents_list)):
        ...
        for instance in range(1, instances + 1):
            ...
            # sys.stdout.flush()
            value_list, step_list = run_single_agent_on_mdp(agents, mdp, episodes, steps, reset_at_terminal=reset_at_terminal, visualize=visualize)
    ...
    plot(value_data, 'Cumulative Reward', agent_color_list, task_name)
    plot(step_data, 'Step', agent_color_list, task_name)
    ...
    for episode in tqdm(range(1, episodes + 1)):
        ...
        for step in range(1, steps + 1):
            ...
            for agent_id in range(agent_num):
                action = agents[agent_id].act(states, reward)
        ...
        # A final update.
        for agent_id in range(agent_num):
            agents[agent_id].act(states, reward, is_final=True) # the high level policy must be updated, no matter whether the last option terminates in the middle or not
        # Reset the MDP, tell the agent the episode is over.
        for agent_id in range(agent_num):
            agents[agent_id].end_of_episode()

        value, step = test_single_agent_on_mdp(agent_num, gamma, agents, mdp, steps, visualize)
...
def test_single_agent_on_mdp(agent_num, gamma, agents, mdp, steps, visualize):
    value = 0.0
    # Compute initial state/reward.
    mdp.reset()
    states = mdp.get_init_states()
    ...
    for step in range(1, steps + 1):
        ...
        action_list = []
        for agent_id in range(agent_num):
            action = agents[agent_id].act(states, reward, learning=False)
            action_list.append(action)
        # Terminal check.
        if mdp.is_goal_state(states):
            print("How could this happen?")
