...
class CentQLearningAgent(Agent):
    ...
    def __init__(self, avai_action_list, agent_num, group_id, name="Centralized Q-learning", alpha=0.1, gamma=0.99, epsilon=0.3, default_q=0.0):
    ...
    def _compute_max_qval_action_pair(self, state, init_list, last_action):
        ...
        elif (not init_list[0]) and init_list[1]:
            max_q_val = float("-inf")
            for action_1 in self.avai_action_list[1]:
                q_val = self.q_func[state[0]][state[1]][last_action[0]][action_1]
                if q_val > max_q_val:
                    max_q_val = q_val
                    best_action_list = [last_action[0], action_1]
        elif init_list[0] and (not init_list[1]):
            max_q_val = float("-inf")
            for action_0 in self.avai_action_list[0]:
                q_val = self.q_func[state[0]][state[1]][action_0][last_action[1]]
                if q_val > max_q_val:
                    max_q_val = q_val
                    best_action_list = [action_0, last_action[1]]
        else:
