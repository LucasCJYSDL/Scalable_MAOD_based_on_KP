...
def _kronecker_product(mdp, A_list, intToS, num_limit, normlized=True, with_degree=True, use_median=False):
    ...
    for init_s in init_list:
        if len(final_init_list) >= num_limit:
            break
        if mdp.is_goal_state_single(intToS[0][init_s[0]]) and mdp.is_goal_state_single(intToS[1][init_s[1]]):
    ...
    for init_s in init_list:
        if len(final_init_list) >= num_limit:
            break
        if mdp.is_goal_state_single(intToS[0][init_s[0]]) or mdp.is_goal_state_single(intToS[1][init_s[1]]):
            final_init_list.append(init_s)
    ...
    for term_s in term_list:
        if len(final_term_list) >= num_limit:
            break
        if mdp.is_goal_state_single(intToS[0][term_s[0]]) and mdp.is_goal_state_single(intToS[1][term_s[1]]):
    ...
    for term_s in term_list:
        if len(final_term_list) >= num_limit:
            break
        if mdp.is_goal_state_single(intToS[0][term_s[0]]) or mdp.is_goal_state_single(intToS[1][term_s[1]]):
            final_term_list.append(term_s)
...
def multi_options(mdp, G_list, intToS, generation_time, num_limit, normalized=True, with_degree=True, use_median=False):
    ...
    for _ in range(generation_time):
        ...
        init_list, term_list, min_S_list, max_S_list = _kronecker_product(mdp, A_list, intToS, num_limit, normlized=normalized, with_degree=with_degree, use_median=use_median)
