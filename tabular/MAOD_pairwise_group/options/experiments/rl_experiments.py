...
def build_option_agent(mdp, option_list, partition_list, intToS_list, agent_num, agent_class, option_type, policy='vi', name='-abstr', visualize_option=False):
    ...
    # multiple_options: [[multi_option_0, multi_option_1,......]_group_0, [multi_option_0, multi_option_1,......]__group_1]
...
def test_offline_agent(args, mdp):
    ...
    for idx in range(agent_num//2):
        pair_multi_matrix, pair_multi_option, pair_multi_partition = GetOption(mdp, k=n_options, intToS=intToS_list[(idx * 2):(idx * 2 + 2)],
                                                                               id_list=[(idx * 2), (idx * 2)+1], num_limit=element_limit,
                                                                               matrix_list=origMatrix_list[(idx * 2):(idx * 2 + 2)], normalized=normalized,
                                                                               method='multiple', with_degree=with_degree, use_median=use_median)
    ...
    single_matrix_list, single_option_list, single_partition_list = GetOption(mdp, k=int(multi_option_num**0.5), matrix_list=origMatrix_list,
                                                                              method='single', with_degree=with_degree, use_median=use_median)
   ...
   #
    ...
    #                   [args.agent+'_multi_option', args.agent + '_single_option', 'Q'],
    #                   ['blue', 'red', 'orange'], args.task + '_' + option_type + '_' + args.agent + '_group',
    ...
    #                   [args.agent + '_multi_option', args.agent + '_single_option', 'Random'],
...
if __name__ == "__main__":
    ...
    parser.add_argument('--task', type=str, default='grid_room6') # grid_fourroom, grid_tworoom
    ...
    parser.add_argument('--agent', type=str, default='DistQ') # Q, Random, DistQ
