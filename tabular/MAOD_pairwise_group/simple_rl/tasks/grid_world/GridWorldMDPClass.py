...
class GridWorldMDP(MDP):
    ...
    def __init__(self, width, height, init_locs, goal_1_locs, goal_2_locs, goal_3_locs, lava_locs, walls,
        ...
        self.width = width
        self.height = height
        self.goal_1_locs = goal_1_locs
        self.goal_2_locs = goal_2_locs
        self.goal_3_locs = goal_3_locs
        self.cur_states = []
        for i in range(self.agent_num):
    ...
    def get_parameters(self):
        ...
        param_dict["goal_1_locs"] = self.goal_1_locs
        param_dict["goal_2_locs"] = self.goal_2_locs
        param_dict["goal_3_locs"] = self.goal_3_locs
    ...
    def is_goal_state(self, states): #c #check
        ...
        for i in range(self.agent_num):
            group_id = i//2
            if group_id == 0:
                if (int(states[i].x), int(states[i].y)) not in self.goal_1_locs:
                    return False
            elif group_id == 1:
                if (int(states[i].x), int(states[i].y)) not in self.goal_2_locs:
                    return False
            else:
                if (int(states[i].x), int(states[i].y)) not in self.goal_3_locs:
                    return False
    ...
    def is_goal_state_single(self, state, agent_id):
        group_id = agent_id // 2
        if group_id == 0:
            return ((int(state.x), int(state.y)) in self.goal_1_locs) and self.is_goal_terminal
        elif group_id == 1:
            return ((int(state.x), int(state.y)) in self.goal_2_locs) and self.is_goal_terminal
        else:
            return ((int(state.x), int(state.y)) in self.goal_3_locs) and self.is_goal_terminal

    def _reward_func_single(self, state, action, agent_id):
        '''
        Args:
            state (State)
            action (str)
        ...
        Returns
            (float)
        '''
        if self._is_goal_state_action_single(state, action, agent_id):
            return 1.0 - self.step_cost
        else:
            return 0 - self.step_cost

    def _is_goal_state_action_single(self, state, action, agent_id):
        '''
        Args:
            state (State)
            action (str)
        ...
        group_id = agent_id // 2
        if group_id == 0:
            temp_goal_locs = self.goal_1_locs
        elif group_id == 1:
            temp_goal_locs = self.goal_2_locs
        else:
            temp_goal_locs = self.goal_3_locs

        if (state.x, state.y) in temp_goal_locs:
            # Already at terminal.
            return False

        if action == 3 and (state.x - 1, state.y) in temp_goal_locs:
            return True
        elif action == 2 and (state.x + 1, state.y) in temp_goal_locs:
            return True
        elif action == 1 and (state.x, state.y - 1) in temp_goal_locs:
            return True
        elif action == 0 and (state.x, state.y + 1) in temp_goal_locs:
            return True
        else:
            return False

    def _transition_func_single(self, state, action, agent_id, allow_further=False):
        '''
        Args:
            state (State)
            action (str)
        ...
        if not allow_further:
            if self.is_goal_state_single(state, agent_id):
                return state, True
        ...
        if self.is_goal_state_single(next_state, agent_id):
            is_terminal = True
        else:
            is_terminal = False
    ...
    def get_goal_1_locs(self):
        return self.goal_1_locs

    def get_goal_2_locs(self):
        return self.goal_2_locs

    def get_goal_3_locs(self):
        return self.goal_3_locs
...
def make_grid_world_from_file(file_name, randomize=False, num_goals=1, name=None, goal_num=None, slip_prob=0.0): #c
    ...
    for i, line in enumerate(wall_lines):
        ...
        for j, ch in enumerate(line):
            if ch == "w":
                ...
                goal_1_locs.append((j + 1, num_rows - i))
            elif ch == "2":
                goal_2_locs.append((j + 1, num_rows - i))
            elif ch == "3":
                goal_3_locs.append((j + 1, num_rows - i))
            elif ch == "l":
    ...
    print("Goal list #1: ", goal_1_locs)
    print("Goal list #2: ", goal_2_locs)
    print("Goal list #3: ", goal_3_locs)
    print("Agent list: ", agent_locs)

    return GridWorldMDP(width=num_cols, height=num_rows, init_locs=agent_locs, goal_1_locs=goal_1_locs,
                        goal_2_locs=goal_2_locs, goal_3_locs=goal_3_locs, lava_locs=lava_locs, walls=walls, name=name, slip_prob=slip_prob)



if __name__ == '__main__':
    grid_world = make_grid_world_from_file(file_name='txt_grids/room6.txt')

    grid_world.reset()
    screen = grid_world.visualize(grid_world.get_curr_states(), filename="room_group")
    #
    # import time
    # action_list = ['down' for _ in range(grid_world.get_agent_num())]
    # for _ in range(10):
    #     grid_world.execute_agent_action(action_list)
    #     screen = grid_world.visualize(grid_world.get_curr_states(), screen)
    #     time.sleep(0.2)

    # from simple_rl.planning.ValueIterationClass import ValueIteration
    # vi = ValueIteration(grid_world, agent_id=4)
    # vi.run_vi()
    # grid_world.visualize_policy(vi.policy, agent_id=4)

    # grid_world.visualize_value(agent_id=4)

    # from simple_rl.abstraction.action_abs.IntrinsitcMDP import IntrinsicMDP
    # from simple_rl.planning.ValueIterationClass import ValueIteration
    # mdp_ = IntrinsicMDP(intrinsic_term_list=[GridWorldState(2,2)], mdp=grid_world)
    # vi = ValueIteration(mdp_, agent_id=2)
    # vi.run_vi()
    # grid_world.visualize_policy(vi.policy, agent_id=2)